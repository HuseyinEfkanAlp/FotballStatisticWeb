...
mevkiSecim = st.sidebar.selectbox('Mevki Seçiniz', ['SeçimYok','Defans','Orta Saha','Forvet'])

if mevkiSecim == "SeçimYok":
...
#Function
def def_rate(def_df,blok = 30,baski = 35, pas = 25, gol = 10, maxYas = 38,minYas=16):
	#Rate
	...
	def_df["Age"] = (2022-def_df["Born"])
	filter_df = def_df[(def_df["Age"]<=maxYas)&(def_df["Age"]>=minYas)]
	if filter_df.shape[0]>0:
		filter_df["YourRate"] = pd.cut(filter_df['Rate'].iloc[:10], 5,labels = [1,2,3,4,5])
		return filter_df
	else:
		return filter_df
...
if mevkiSecim == 'Defans':
	...
	yas = st.sidebar.slider("Yaş Aralığı Seçiniz",16,38,value=(16, 38))
	st.sidebar.write("Seçtiğiniz Yaş Aralığı:", yas)
	#Bilgilendirme
	if toplamdef == 100:
		...
		top10defans = def_rate(def_df = defans_df, blok=defansdict["Blok"],baski=defansdict["Baskı"],\
			pas=defansdict["Pass"], gol=defansdict["Gol"],maxYas=yas[1],minYas=yas[0])
		if top10defans.shape[0]>0:
			st.subheader(f"Ağırlıklarınıza göre 2021-2022 sezonunun en iyi defansı/defansları")
			st.table(top10defans.iloc[0:10,[2,3,4,5,6,7,8,14,15]])
			...
			st.error("Aradığınız yaşta oyuncu bulunamadı. Lütfen yaş aralığını değiştirin")
	else:
		st.sidebar.error(f"Ağırlıkların Toplamı 100 Olmalı. Ağırlıklarınızın Toplamı :{toplamdef}")
...
#--
# Function
def for_rate(for_df,press = 15,pas = 15, sut = 25, gol = 30,dripling=15, maxYas = 38,minYas=16):
	#Rate
	...
	for_df["Age"] = (2022-for_df["Born"])
	filter_df = for_df[(for_df["Age"]<=maxYas)&(for_df["Age"]>=minYas)]
	if filter_df.shape[0]>0:
		filter_df["YourRate"] = pd.cut(filter_df['Rate'].iloc[:10], 5,labels = [1,2,3,4,5])
		return filter_df
	else:
...
#--
# Function
def ort_rate(orta_df,press = 15,pas = 15, sut = 25, gol = 30,dripling=15, maxYas = 38,minYas=16):
	#Rate
	...
	orta_df["Age"] = (2022-orta_df["Born"])
	filter_df = orta_df[(orta_df["Age"]<=maxYas)&(orta_df["Age"]>=minYas)]
	if filter_df.shape[0]>0:
		filter_df["YourRate"] = pd.cut(filter_df['Rate'].iloc[:10], 5,labels = [1,2,3,4,5])
		return filter_df
	else:
